# BetterEnd Sky [ABANDONED]
![license: LGPL-3.0](https://img.shields.io/badge/license-LGPL--3.0-green?style=flat-square)
![environment: client](https://img.shields.io/badge/environment-client-1976d2?style=flat-square)
![modloader: Fabric](https://img.shields.io/badge/modloader-Fabric-1976d2?style=flat-square&logo=data:image/png;base64,)

**This mod has ceased development. Please use [PinkGoosik's](https://github.com/PinkGoosik/better-end-sky) mod for 1.18+ Minecraft.**

A Fabric mod which adds the fabulous End sky from the BetterEnd mod.\
BetterEnd Sky requires [BCLib](https://www.curseforge.com/minecraft/mc-mods/bclib) and [Fabric API](https://www.curseforge.com/minecraft/mc-mods/fabric-api).

## Screenshots
Props to team BetterX for their amazing work which looks even better with shaders.
![vanilla](https://i.imgur.com/nSjX1l5.png)
![shaders](https://i.imgur.com/PerkDIm.png)
![vanilla](https://i.imgur.com/Va3Xz9h.png)
![shaders](https://i.imgur.com/uYiGQSv.png)

Shaders used: [Complementary v4.0.2](https://www.curseforge.com/minecraft/customization/complementary-shaders)
